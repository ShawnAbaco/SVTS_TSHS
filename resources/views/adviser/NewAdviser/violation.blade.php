...
@section('content')
    <div class="main-container">
        ...
        <div class="select-options">
            ...
            <div class="right-controls">
                <button class="btn-referral" id="referralBtn">🔄 Refer to Prefect</button> <!-- Added this line -->
                    ...
                    @if ($viewType == 'group' || $viewType == 'group_per_offense')
                        ...
                        @forelse($byGroupViolations as $group)
                            <tr data-group-key="{{ $group->group_key }}" data-incident="{{ $group->violation_incident }}"
                                data-handled-by="{{ $group->handled_by ?? 'adviser' }}"
                    ...
                    @else
                        ...
                        @forelse($violations as $violation)
                            ...
                            <tr data-violation-id="{{ $violation->violation_id }}"
                                data-handled-by="{{ $violation->handled_by ?? 'adviser' }}"
            ...
            <div class="modal-content">
                ...
                <div class="edit-modal-body">
                    <form id="setScheduleForm" method="POST"
                        action="{{ route('adviser.storeMultipleAppointments') }}">
                        @csrf
        ...
        <div class="modal" id="updateSanctionModal">
            <div class="modal-content">
                ...
                <div class="edit-modal-body expanded">
                    <form id="updateSanctionForm" method="POST" action="{{ route('adviser.updateSanction') }}">
                        @csrf
            ...
            <div class="modal-content">
                ...
                <div class="edit-modal-body">
                    <form id="createAnecdotalForm" method="POST"
                        action="{{ route('adviser.storeMultipleAnecdotals') }}">
                        @csrf
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function generateExcelFile(data, selectedStatus, searchTerm, recordCount) {
                ...
                XLSX.utils.sheet_add_aoa(ws, [
                    ...
                    [
                        'Prepared By: {{ Auth::user()->adviser_fname }} {{ Auth::user()->adviser_lname }}'
                    ],
                    ['']
                ], {
                    origin: -1
                ...
                settleButtons.forEach(btn => {
                    btn.addEventListener('click', function(e) {
                        ...
                        settlementForm.action =
                            `/adviser/violations/update/${currentViolationData.violationId}`;
        ...
        // ==========================
        // 🔄 REFERRAL TO PREFECT FUNCTIONALITY
        // ==========================
        ...
        document.getElementById('referralBtn').addEventListener('click', async function() {
            ...
            if (!selectedCheckboxes.length) {
                notifications.showNotification('Please select at least one violation to refer to prefect.',
                    'warning');
                return;
            }

            // Check if all selected violations are currently handled by adviser
            const violationsToRefer = [];
            const violationsAlreadyWithPrefect = [];
            ...
            selectedCheckboxes.forEach(cb => {
                ...
                const violationData = {
                    ...
                    is_group: isGroup,
                    group_key: isGroup ? cb.value : null,
                    current_handled_by: row.getAttribute('data-handled-by') ||
                        'adviser' // Assuming you add this attribute
                };

                // Check if already handled by prefect
                if (violationData.current_handled_by === 'prefect') {
                    violationsAlreadyWithPrefect.push(violationData);
                } else {
                    violationsToRefer.push(violationData);
                }
            });

            // Show warning if some violations are already with prefect
            if (violationsAlreadyWithPrefect.length > 0) {
                let message = `${violationsAlreadyWithPrefect.length} violation(s) are already with prefect: `;

                if (violationsAlreadyWithPrefect.length <= 3) {
                    const names = violationsAlreadyWithPrefect.map(v => v.student_name).join(', ');
                    message += names;
                } else {
                    message += `${violationsAlreadyWithPrefect.length} violations`;
                }

                notifications.showNotification(message, 'warning');

                // If all are already with prefect, return
                if (violationsToRefer.length === 0) {
                    return;
                }
            }
            ...
            const confirmMessage =
                `Refer ${violationsToRefer.length} violation(s) to prefect? This will update the handled_by field to "prefect".`;

            notifications.showConfirmation(confirmMessage, async () => {
                // Proceed with referral
                await processReferral(violationsToRefer);
            });
        });

        async function processReferral(violations) {
            const referralBtn = document.getElementById('referralBtn');
            const originalText = referralBtn.innerHTML;
            referralBtn.innerHTML = '<span>⏳ Referring...</span>';
            ...
            try {
                ...
                // Separate individual and group violations
                const individualViolations = violations.filter(v => !v.is_group);
                const groupViolations = violations.filter(v => v.is_group);

                // Add individual violation IDs
                individualViolations.forEach(v => {
                    formData.append('violation_ids[]', v.violation_id);
                });

                // Add group keys
                groupViolations.forEach(v => {
                    formData.append('group_keys[]', v.group_key);
                });

                console.log('Referring violations:', {
                    individual: individualViolations.length,
                    group: groupViolations.length
                });

                // Send the request
                const response = await fetch('{{ route('adviser.violations.referToPrefect') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Accept': 'application/json'
                ...
                const result = await response.json();
                console.log('Referral response:', result);

                if (result.success) {
                    notifications.showNotification(`✅ ${result.message}`, 'success');

                    // Update the UI
                    updateHandledByStatus(violations, 'prefect');
                    ...
                    notifications.showNotification(`❌ ${result.message}`, 'error');
                }
            } catch (error) {
                console.error('Referral error:', error);
                notifications.showNotification('❌ An error occurred during referral.', 'error');
            } finally {
                referralBtn.innerHTML = originalText;
                referralBtn.disabled = false;
            }
        }

        function updateHandledByStatus(violations, newStatus) {
            violations.forEach(violation => {
                if (!violation.is_group) {
                    // Update individual violation row
                    const row = document.querySelector(`tr[data-violation-id="${violation.violation_id}"]`);
                    if (row) {
                        row.setAttribute('data-handled-by', newStatus);
                        // Add visual indicator
                        const statusCell = row.querySelector('td:nth-child(9)'); // Actions column
                        if (statusCell) {
                            const handledBadge = document.createElement('span');
                            handledBadge.className = 'handled-badge';
                            handledBadge.textContent = newStatus === 'prefect' ? '👮 Prefect' : '👨‍🏫 Adviser';
                            handledBadge.style.marginLeft = '5px';
                            handledBadge.style.fontSize = '0.8em';
                            handledBadge.style.padding = '2px 6px';
                            handledBadge.style.borderRadius = '3px';
                            handledBadge.style.backgroundColor = newStatus === 'prefect' ? '#9c27b0' : '#3b82f6';
                            handledBadge.style.color = 'white';

                            // Remove existing badge if any
                            const existingBadge = statusCell.querySelector('.handled-badge');
                            if (existingBadge) {
                                existingBadge.remove();
                            }

                            statusCell.appendChild(handledBadge);
                        }
                    }
                } else {
                    // Update group rows (if needed)
                    const rows = document.querySelectorAll(`tr[data-group-key="${violation.group_key}"]`);
                    rows.forEach(row => {
                        row.setAttribute('data-handled-by', newStatus);
                    });
                }
        ...
        // Optional: Add a visual indicator for prefect-handled violations
        function addHandledByIndicators() {
            const rows = document.querySelectorAll('#tableBody tr');
            rows.forEach(row => {
                const handledBy = row.getAttribute('data-handled-by');
                if (handledBy === 'prefect') {
                    const statusCell = row.querySelector('td:nth-child(9)');
                    if (statusCell && !statusCell.querySelector('.handled-badge')) {
                        const badge = document.createElement('span');
                        badge.className = 'handled-badge';
                        badge.textContent = '👮 Prefect';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Add handled-by indicators
            addHandledByIndicators();

            // Add data-handled-by attribute to rows if not present
            const rows = document.querySelectorAll('#tableBody tr');
            rows.forEach(row => {
                if (!row.hasAttribute('data-handled-by')) {
                    row.setAttribute('data-handled-by', 'adviser');
                }
            });
        });
    </script>
    ...
    <style>
        /* Add this to the existing button styles section */
        .btn-referral {
            padding: 10px 16px;
            border: none;
            border-radius: 6px;
