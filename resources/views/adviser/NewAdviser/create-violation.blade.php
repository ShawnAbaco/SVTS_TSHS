        ...
        <div class="nav-breadcrumb">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('adviser.violation') }}"
...
<script>
    ...
    async function getStudentOffenseHistoryForOffense(studentId, offenseId) {
        try {
            const response = await fetch('{{ route("adviser.violations.get-offense-history") }}', {
                method: 'POST',
                headers: {
    ...
    async function loadAdditionalOffenseSanctions() {
        ...
        try {
            ...
            const response = await fetch('/adviser/violations/get-sanction-stages', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    async function finalSubmit() {
        ...
        try {
            const response = await fetch('{{ route('adviser.violations.store') }}', {
                method: 'POST',
                headers: {
            ...
            if (contentType && contentType.includes('application/json')) {
                const data = await response.json();
                if (response.ok) {
                    window.location.href = '{{ route('adviser.violation') }}';
                } else {
    ...
    async function createOffensesReviewSection() {
        ...
        studentCards.forEach(studentCard => {
            ...
            offenseEntries.forEach((entry, index) => {
                ...
                const previousSanction = entry.querySelector('.offense-details .small:nth-child(2)')?.textContent?.replace('Previous sanction:', '').trim() || 'None';
                const currentSanction = entry.querySelector('.offense-details .small:nth-child(3) .text-success')?.textContent?.trim() || 'Verbal Warning';
