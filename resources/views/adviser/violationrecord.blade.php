...
<div class="select-options">
  ...
  <div class="right-controls">
    ...
    <div id="violationRecordsActions" class="action-buttons">
      <button class="btn-cleared" id="referToPrefectBtn">📚 Refer to Prefect</button>
...
<div class="table-container">
    ...
    <div id="violationRecordsTable" class="table-wrapper">
      <table>
        ...
        <tbody id="tableBody">
          @if($viewType == 'group')
    <!-- GROUP VIEW - SHOW ALL STATUSES -->
    @forelse($byGroupViolations as $group)
        @php
            // Determine status class and display text for group
            $statusClass = 'status-pending';
            $statusText = 'Pending';

            if (isset($group->status)) {
                $statusClass = 'status-' . $group->status;
                $statusText = ucfirst(str_replace('_', ' ', $group->status));
            }

            // Define which statuses should have enabled checkboxes
            $enabledStatuses = ['pending', 'referred_to_prefect', 'under_review', 'active', 'rescheduled', 'needs_review'];
            $isCheckboxEnabled = in_array($group->status ?? 'pending', $enabledStatuses);
        @endphp
        ...
        >
            <td>
                <input type="checkbox" class="rowCheckbox groupCheckbox"
                       value="{{ $group->group_key }}"
                       data-type="group"
                       @if(!$isCheckboxEnabled) disabled @endif>
            </td>
            <td>
    ...
    @endforelse
@else
    <!-- INDIVIDUAL VIEW - SHOW ALL STATUSES -->
    @forelse($violations as $violation)
        @php
            ...
            $statusClass = 'status-' . $violation->status;
            $statusText = ucfirst(str_replace('_', ' ', $violation->status));

            // Define which statuses should have enabled checkboxes
            $enabledStatuses = ['pending', 'referred_to_prefect', 'under_review', 'active', 'rescheduled', 'needs_review'];
            $isCheckboxEnabled = in_array($violation->status, $enabledStatuses);
        @endphp
        ...
        <div class="pagination-links">
          @if($viewType == 'individual' && $violations instanceof \Illuminate\Pagination\LengthAwarePaginator)
            ...
            <div class="pagination-simple">
              ...
              @else
                <a href="{{ $violations->previousPageUrl() }}" class="pagination-link">Previous</a>
              @endif

              {{-- Next Page Link --}}
              @if ($violations->hasMorePages())
                <a href="{{ $violations->nextPageUrl() }}" class="pagination-link">Next</a>
              @else
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <select id="edit_violation_status" name="status" required>
              ...
              <option value="referred_to_prefect">Referred to Prefect</option>
  ...
  <div class="modal-content large-modal">
    ...
    <div class="modal-body">
      ...
      <div class="archive-table-container no-scroll">
        <div class="archive-table-wrapper" id="archiveViolationRecordsTable">
          <table class="archive-table">
            <thead>
              <tr>
                <th class="select-label">
                  <label>
            ...
            <tbody id="archiveViolationRecordsBody"></tbody>
          </table>
        </div>
      </div>
...
// Select All for Violation Records - Only enabled checkboxes
document.getElementById('selectAll').addEventListener('change', function() {
    const checkboxes = document.querySelectorAll('.violationCheckbox:not(:disabled), .groupCheckbox:not(:disabled)');
    checkboxes.forEach(cb => {
        cb.checked = this.checked;
    });
});

// Function to get only enabled checkboxes
function getSelectedEnabledCheckboxes() {
    return document.querySelectorAll('.violationCheckbox:checked:not(:disabled), .groupCheckbox:checked:not(:disabled)');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function openViolationModal(action, data, isGroup = false) {
        ...
        document.getElementById('edit_violation_status').value = data.status || 'pending';
    ...
    // Add click event listeners to all rows for edit functionality
    document.addEventListener('DOMContentLoaded', function() {
        const allRows = document.querySelectorAll('#violationRecordsTable tbody tr.clickable-row');

        allRows.forEach(row => {
            row.addEventListener('click', function(e) {
                // Don't trigger if clicking on checkbox or edit button
                if (e.target.type === 'checkbox' ||
                    e.target.classList.contains('editViolationBtn') ||
                    e.target.classList.contains('editGroupBtn')) {
                    return;
                }

                // Handle individual rows
                if (this.classList.contains('individual-row')) {
                    openViolationModal(`/adviser/violations/update/${this.dataset.violationId}`, {
                        id: this.dataset.violationId,
                        studentId: this.dataset.studentId,
                        incident: this.dataset.incident,
                        date: this.dataset.date,
                        time: this.dataset.time,
                        offenseId: this.dataset.offenseId,
                        status: this.dataset.status
                    }, false);
                }
                // Handle group rows
                else if (this.classList.contains('group-row')) {
    ...
    function openGroupEditModal(groupKey, groupData) {
        // For now, we'll use a confirmation and redirect to individual view
        notifications.showConfirmation(
            'Editing a group will affect all students in this group. Do you want to view the individual violations in this group for editing?',
            function() {
                // Redirect to individual view with this group filtered
                const url = new URL(window.location.href);
                url.searchParams.set('view', 'individual');
                url.searchParams.set('group', groupKey);
                window.location.href = url.toString();
...
document.getElementById('setScheduleBtn').addEventListener('click', function() {
    const selectedCheckboxes = getSelectedEnabledCheckboxes();

    if (!selectedCheckboxes.length) {
        notifications.showNotification('Please select at least one pending violation to schedule.', 'warning');
        return;
    }
...
document.getElementById('createAnecdotalBtn').addEventListener('click', function() {
    // Get both individual and group checkboxes - only enabled ones
    const selectedIndividual = document.querySelectorAll('.violationCheckbox:checked:not(:disabled)');
    const selectedGroup = document.querySelectorAll('.groupCheckbox:checked:not(:disabled)');

    const totalSelected = selectedIndividual.length + selectedGroup.length;

    if (!totalSelected) {
        notifications.showNotification('Please select at least one pending violation to create anecdotal record.', 'warning');
        return;
    }
...
document.getElementById('printAnecdotalBtn').addEventListener('click', function() {
    ...
    const printUrl = `/adviser/violations/generate-multiple-anecdotal-pdf?violation_ids=${selectedViolationIds.join(',')}`;
...
document.getElementById('printAnecdotalBtn').addEventListener('click', function() {
    ...
    // Generate PDF using the violation IDs
    const printUrl = `/adviser/violations/generate-multiple-anecdotal-pdf?violation_ids=${window.selectedViolationIds.join(',')}`;
...
document.getElementById('moveToTrashBtn').addEventListener('click', async function() {
    ...
    if (!selectedCheckboxes.length) {
        notifications.showNotification('Please select at least one pending violation.', 'warning');
        return;
    }
...
// ✅ Refer to Prefect Functionality - FIXED
document.getElementById('referToPrefectBtn').addEventListener('click', async function() {
    const selectedCheckboxes = getSelectedEnabledCheckboxes();

    if (!selectedCheckboxes.length) {
        notifications.showNotification('Please select at least one pending violation to refer to Prefect.', 'warning');
        return;
    }

    // Collect selected violations and groups - FIXED PARAMETER NAMES
    const selectedData = {
        complaint_ids: [],  // ✅ CORRECT - matches controller expectation
        group_keys: []
    };

    selectedCheckboxes.forEach(cb => {
        if (cb.classList.contains('violationCheckbox')) {
            selectedData.complaint_ids.push(cb.value);  // ✅ CORRECT
        } else if (cb.classList.contains('groupCheckbox')) {
            selectedData.group_keys.push(cb.value);
        }
    });

    console.log('Sending data to refer-to-prefect:', selectedData); // Debug log

    notifications.showConfirmation(
        `Are you sure you want to refer ${selectedData.complaint_ids.length + selectedData.group_keys.length} violation(s) to the Prefect?`,
        async function() {
            try {
                const response = await fetch('/adviser/violations/refer-to-prefect', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken,
                ...
                const result = await response.json();
                console.log('Refer to Prefect response:', result); // Debug log

                if (result.success) {
    notifications.showNotification(`${result.processed_count || (selectedData.complaint_ids.length + selectedData.group_keys.length)} violation(s) referred to Prefect successfully.`, 'success');
    ...
    notifications.showNotification('Error: ' + errorMessage, 'error');
}
            } catch (error) {
                console.error('Error:', error);
                notifications.showNotification('Error referring violations to Prefect.', 'error');
            }
        }
    );
});
