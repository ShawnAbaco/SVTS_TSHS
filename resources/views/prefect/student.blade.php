...
@section('content')
        ...
        <div class="grade-filter-section"
            ...
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div style="margin-left: auto; font-size: 13px; color: #6b7280;">
                    <span id="filterCount">{{ $students->count() }}</span> students shown
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function filterTableByGrade(selectedGrade) {
                ...
                rows.forEach(row => {
                    ...
                    const gradeNumber = grade.toString().replace('Grade ', '');
                ...
                if (visibleCount === 0) {
                    ...
                    if (!noDataRow) {
                        ...
                        newRow.innerHTML =
                            '<td colspan="8" class="no-data">⚠️ No students found for this grade level</td>';
            ...
            function filterTable() {
                ...
                if (visibleCount === 0) {
                    ...
                    if (!noDataRow) {
                        const tbody = document.getElementById('tableBody');
                        const newRow = document.createElement('tr');
                        newRow.innerHTML = '<td colspan="8" class="no-data">⚠️ No students found</td>';
                        tbody.appendChild(newRow);
                    }
                } else {
                    const noDataRow = document.querySelector('#tableBody .no-data');
        ...
        async function loadAdvisers() {
            try {
                const response = await fetch('/prefect/advisers/all', {
                    headers: {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function saveStudentChanges() {
                ...
                fetch('{{ route('prefect.students.update') }}', {
        ...
        function exportAllStudents(format) {
            ...
            if (selectedGrade === '11') {
                reportTitle = 'Grade 11 Students';
            } else if (selectedGrade === '12') {
                reportTitle = 'Grade 12 Students';
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function loadViolations(studentId) {
                try {
                    ...
                    const response = await fetch(`/prefect/students/${studentId}/violationsforstudent`, {
                        headers: {
                            'X-CSRF-TOKEN': csrfToken,
                            'Accept': 'application/json'
                        }
            ...
            async function exportViolationsPDF() {
                ...
                try {
                    const response = await fetch(
                        `/prefect/students/${currentStudentData.id}/violationsforstudent`, {
                            headers: {
                                'X-CSRF-TOKEN': csrfToken,
                                'Accept': 'application/json'
                            }
