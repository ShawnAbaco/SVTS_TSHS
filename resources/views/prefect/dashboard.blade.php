            ...
            <div class="table-wrapper">
                <table class="violations-table">
                    ...
                    <tbody id="violationsTableBody">
                        @php
                            $violationsArray =
                                $violations instanceof \Illuminate\Support\Collection
                                    ? $violations->toArray()
                                    : (array) $violations;
                            ...
                            @foreach ($paginatedViolations as $index => $violation)
                                @php
                                    if (is_object($violation)) {
                                        $studentName =
                                            $violation->student_name ?? ($violation['student_name'] ?? 'N/A');
                                        $gradeLevel = $violation->grade_level ?? ($violation['grade_level'] ?? 'N/A');
                                        $violationType =
                                            $violation->violation_type ?? ($violation['violation_type'] ?? 'N/A');
                                ...
                                <tr>
                                        ...
                                        <div class="student-info">
                                            ...
                                            <div>
                                                <div class="student-name">
                                                    {{ \Illuminate\Support\Str::limit($studentName, 20) }}</div>
                                    ...
                                    <td>
                                        <span
                                            class="violation-type">{{ \Illuminate\Support\Str::limit($violationType, 15) }}</span>
                                    </td>
                                    <td>
                                        <div class="violation-date">{{ \Carbon\Carbon::parse($date)->format('M d, Y') }}
                                        </div>
                                    </td>
                                    <td>
                                        @if ($status == 'pending')
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <button class="btn-action view"
                                                onclick="viewViolationDetails({{ $index + $startIndex }})"
                                                title="View Details">
