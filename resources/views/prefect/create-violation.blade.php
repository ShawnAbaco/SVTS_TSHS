...
.recent-violations .list-group-item:last-child {
    border-bottom: 0;
        ...
        /* NEW: Style for offense description in cards */
        .offense-entry .offense-description {
            font-size: 0.8rem;
            color: #6c757d;
            line-height: 1.4;
            margin-top: 0.25rem;
        ...
        /* Make the description italics */
        .offense-entry .text-muted i {
            color: #6c757d;
            font-size: 0.75rem;
        }

        /* Truncate long descriptions */
        .offense-entry .text-muted {
            max-height: 3rem;
            overflow: hidden;
        ...
        .offense-entry:hover .text-muted {
            max-height: none;
            -webkit-line-clamp: unset;
        }

        /* Style for offense description in list */
        .offense-item .offense-description-preview {
            font-size: 0.8rem;
            margin-top: 0.25rem;
            line-height: 1.3;
            max-height: 2rem;
                                                ...
                                                <div id="offense_results">
                                                    @foreach ($offenses as $offense)
                                                        <div class="offense-item"
                                                            ...
                                                            data-offense-description="{{ $offense->offense_description ? addslashes($offense->offense_description) : '' }}"
                                                            onclick="toggleOffense(this)">
                                                            <div class="d-flex align-items-center">
                                                                <div class="flex-grow-1">
                                                                    <strong>{{ $offense->offense_type }}</strong>
                                                                    @if($offense->offense_description)
                                                                        <div class="offense-description-preview">
...
<script>
    ...
    // UPDATED: Include offense description in the data
    let allOffenses = [
        @foreach ($offenses as $offense)
            {
                id: "{{ $offense->offense_id }}",
                type: "{{ $offense->offense_type }}",
                description: "{{ $offense->offense_description ? addslashes($offense->offense_description) : '' }}"
            },
        @endforeach
    ];
    ...
    function searchOffenses() {
        ...
        if (searchTerm === '') {
            ...
            const filteredOffenses = allOffenses.filter(offense =>
                ...
                (offense.description && offense.description.toLowerCase().includes(searchTerm))
            );
    ...
    function renderOffenseList(offenses) {
        ...
        offenses.forEach(offense => {
            ...
            offenseItem.setAttribute('data-offense-description', offense.description || '');
            offenseItem.onclick = () => toggleOffense(offenseItem);

            // Show description in the list if available
            const descriptionHTML = offense.description ?
                `<div class="offense-description-preview">${offense.description}</div>` : '';
    ...
    function toggleOffense(element) {
        ...
        const offenseDescription = element.getAttribute('data-offense-description') || '';
    ...
    function updateSelectedOffensesDisplay() {
        ...
        selectedOffenses.forEach(offense => {
            ...
            if (offense.description) {
                ...
                tag.setAttribute('title', offense.description);
            }
    ...
    // ==========================================
    // UPDATED: CREATE STUDENT HISTORY DIV WITH OFFENSE DESCRIPTION
    // ==========================================
    ...
    // ==========================================
    // UPDATED: CREATE OFFENSE ENTRIES WITH DESCRIPTION
    // ==========================================
    async function createOffenseEntries(studentId, student, count) {
        ...
        for (const offense of selectedOffensesData) {
            ...
            const offenseDescription = offense.description || '';
            ...
            // NEW: Include offense description if available
            const descriptionHTML = offenseDescription ?
                `<div class="small mb-2 text-muted">
                    <i class="fas fa-info-circle me-1"></i>${offenseDescription}
                </div>` : '';
    ...
    async function loadAdditionalOffenseSanctions() {
        ...
        try {
            ...
            const offenseCount = studentHistory ? getOffenseSpecificCountFromHistory(studentHistory, offenseId) : 0;
    ...
    async function updateReviewModalContent() {
        ...
        try {
            // Check if offense history has been loaded
            const hasHistory = document.getElementById('offenseHistoryContent').children.length > 0;

            if (!hasHistory && selectedOffenses.length > 0 && violationPairs.length > 0) {
                // Try to load offense history if not already loaded
                reviewList.innerHTML = '<div class="text-center"><i class="fas fa-spinner fa-spin me-2"></i>Loading offense history for accurate review...</div>';

                // Simulate a small delay to allow any pending history loads
    ...
    function updateReviewStatistics() {
        ...
        document.getElementById('reviewCustomSanctions').textContent = additionalOffenses.length;
    ...
    // Helper functions that need to be defined but are called from other functions
    ...
    async function getRecommendedSanction(studentId, offenseId) {
        try {
            ...
            console.error('Error getting recommended sanction:', error);
            return 'Verbal Warning';
        }
    }
    ...
    async function createOffensesReviewSection() {
        ...
        // Get all student cards from the offense preview
        const studentCards = document.querySelectorAll('.student-history-card');

        if (studentCards.length === 0) {
            // If no cards are displayed yet, use the old method as fallback
            console.warn('No student cards found in preview, using fallback method');
            return await createOffensesReviewSectionFallback(selectedOffensesData);
        }

        // Loop through each student card in the offense preview
        studentCards.forEach(studentCard => {
            ...
            const gradeLevel = student.adviser?.adviser_gradelevel || 'N/A';
            const section = student.adviser?.adviser_section || '';
            const fullStudentName = `${studentName} - Gr. ${gradeLevel} ${section}`.trim();

            // Find all offense entries for this student
            const offenseEntries = studentCard.querySelectorAll('.offense-entry');

            offenseEntries.forEach((entry, index) => {
                // Extract data from the offense entry
                const offenseText = entry.querySelector('.offense-details .small:nth-child(1)')?.textContent?.replace('Offense:', '').trim() || 'Unknown Offense';
                const previousSanction = entry.querySelector('.offense-details .small:nth-child(3)')?.textContent?.replace('Previous sanction:', '').trim() || 'None';
                const currentSanction = entry.querySelector('.offense-details .small:nth-child(4) .text-success')?.textContent?.trim() || 'Verbal Warning';

                // For the review table, "Recommended Sanction" should be the same as "Current Sanction"
                const recommendedSanction = currentSanction;

                html += `
                    <tr>
                        <td><strong>${rowCount}</strong></td>
                        <td>${fullStudentName}</td>
                        <td>${offenseText}</td>
                        <td>${previousSanction}</td>
                        <td>
                            <strong class="text-success">${recommendedSanction}</strong>
                        </td>
                    </tr>
                `;
                rowCount++;
        ...
        for (const offense of additionalOffenses) {
            html += `
                <tr class="table-info">
                    ...
                    <td>
                        <strong class="text-success">${offense.recommended_sanction || offense.sanction_text}</strong>
                    </td>
                </tr>
            `;
            rowCount++;
    ...
    async function createOffensesReviewSectionFallback(selectedOffensesData) {
        let html = `
            <div class="table-responsive">
                <table class="table table-bordered table-sm review-table">
                    <thead class="table-light">
        ...
        // Fallback: Use the existing method but get current sanction instead of recommended
        for (const pair of violationPairs) {
            ...
            for (const offense of selectedOffensesData) {
                ...
                // Get current sanction (same as what's shown in offense preview)
                const currentSanction = await getCurrentSanction(studentId, offenseId);
                const studentHistory = await getStudentOffenseHistoryForOffense(studentId, offenseId);
                const previousSanction = studentHistory ? getPreviousSanctionForOffense(studentHistory, offenseId) : 'None';
                ...
                html += `
                    <tr>
                        ...
                        <td>${previousSanction}</td>
                        <td>
                            <strong class="text-success">${currentSanction}</strong>
                        </td>
                    </tr>
                `;
                rowCount++;
        ...
        html += `
            ...
            <div class="alert alert-warning mt-2">
                <i class="fas fa-exclamation-triangle me-1"></i>
                Using fallback method. Make sure to generate violator offense records first for accurate data.
            </div>
        `;
    ...
    function updateOffenseHistoryDisplay() {
        // This function is called but doesn't need implementation since we have loadOffenseHistoryManually()
        console.log('Offense history display would update here');
    }
</script>
</body>
