...
document.getElementById('moveToTrashBtn').addEventListener('click', () => {
  ...
  if (selected.length === 0) {
    showNotification('⚠️ No Selection', 'Please select at least one complaint record to move to trash.', 'warning', {
      yesText: 'OK',
      noText: null,
      onYes: () => {
    ...
    showNotification('🗑️ Move to Trash', `Are you sure you want to move ${selected.length} complaint record(s) to trash?`, 'confirm', {
      yesText: 'Yes, Move',
      noText: 'Cancel',
      onYes: () => {
        // AJAX call to move to trash
        setTimeout(() => {
          showNotification('✅ Success', `${selected.length} complaint record(s) moved to trash successfully.`, 'success', {
            yesText: 'OK',
            noText: null,
            onYes: () => {
              document.getElementById('notificationModal').style.display = 'none';
...
document.getElementById('sendSmsBtn').addEventListener('click', () => {
  showNotification('📩 Send SMS', 'Are you sure you want to send an SMS about this complaint?', 'confirm', {
    yesText: 'Send SMS',
    noText: 'Cancel',
    onYes: () => {
...
document.querySelectorAll('.editBtn').forEach(btn => {
  btn.addEventListener('click', e => {
    ...
    const data = row.dataset.details.split('|');
    showNotification('✏️ Edit Complaint', `Edit complaint for student: ${data[0]}`, 'info', {
      yesText: 'OK',
      noText: null,
      onYes: () => {
        document.getElementById('notificationModal').style.display = 'none';
