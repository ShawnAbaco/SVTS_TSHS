...
@section('content')
    <div class="offenses-sanctions-container">
        ...
        <div class="offenses-table-section">
            <div class="offenses-section-header">
                <div class="offenses-section-title">
                    <h3><i class="fas fa-balance-scale"></i> Sanctions List</h3>
                </div>
                <div class="offenses-section-actions">
                    <button class="offenses-export-btn"
                        onclick="window.open('{{ asset('pdf/SANCTIONS-LIST.pdf') }}', '_blank')">
                        <i class="fas fa-file-pdf"></i> Export PDF
                    </button>
                </div>
            </div>
            <table id="sanctionTable" class="offenses-table">
                <thead>
                    <tr>
                        <th>#</th>
                ...
                <tbody id="sanctionTableBody">
                    @forelse ($allSanctions as $sanction)
                        <tr data-details="{{ $sanction->sanction_consequences }}|{{ $sanction->sanction_description }}">
                            <td>{{ $loop->iteration }}</td>
                            <td>
                                <div class="offenses-sanction-consequences">
                                    <span class="offenses-sanction-icon"><i
                                            class="fas fa-exclamation-triangle"></i></span>
                                    {{ $sanction->sanction_consequences }}
                                </div>
                            </td>
                            <td>{{ $sanction->sanction_description }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="offenses-no-data">
                                <div class="offenses-empty-state-sm">
                                    <i class="fas fa-exclamation-triangle"></i>
        ...
        <script>
            ...
            async function loadSanctionsDropdownByOffense(offenseType) {
                try {
                    ...
                    const response = await fetch(
                        `/prefect/offensesandsanctions/sanctions-by-offense/${encodedOffenseType}`, {
                            headers: {
            ...
            async function loadAllSanctionsDropdown(sanctionSelect = null) {
                try {
                    ...
                    const response = await fetch('/prefect/offensesandsanctions/sanctions-dropdown', {
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json'
        ...
        `;
                    ...
                    const url =
                        `/prefect/offensesandsanctions/all-violations/${encodedOffenseType}${queryString ? '?' + queryString : ''}`;
    ...
    `;
        ...
        `;
                    ...
                    const encodedOffenseType = encodeURIComponent(offenseType);
                    const response = await fetch(`/prefect/offensesandsanctions/details/${encodedOffenseType}`, {
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json'
        ...
        `;
                    ...
                    const response = await fetch(`/prefect/students/${studentId}/violations`, {
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json'
