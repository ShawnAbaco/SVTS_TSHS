...
// In PViolationController.php, update the markAllNotificationsAsRead method:

public function markAllNotificationsAsRead(Request $request)
{
    try {
        \Log::info('Mark all notifications as read called');

        $user = Auth::guard('prefect')->user();

        if (!$user) {
            \Log::warning('Unauthorized access to mark notifications as read');
        ...
        \Log::info('User authenticated: ' . $user->prefect_fname . ' ' . $user->prefect_lname);

        // Since these notifications are just UI-based (not stored in database),
...
// Also update the deleteReadNotifications method:

public function deleteReadNotifications(Request $request)
{
    try {
        \Log::info('Delete read notifications called');

        $user = Auth::guard('prefect')->user();

        if (!$user) {
            \Log::warning('Unauthorized access to delete read notifications');
            return response()->json([
                'success' => false,
                'message' => 'Unauthorized'
            ], 401);
        ...
        // Since these notifications are just UI-based,
        // we can just return success
        return response()->json([
            'success' => true,
        ...
        \Log::error('Error in deleteReadNotifications: ' . $e->getMessage());
        \Log::error('Stack trace: ' . $e->getTraceAsString());
...
private function getNotificationData()
{
    ...
    return [
        'notificationCount' => $notificationCount,
                'newViolationsCount' => $totalViolationsCount, // Add this key
...
public function index(Request $request)
{
    ...
    // Get the actual dates from your violation records - ONLY PRECEF HANDLED, exclude inactive
    ...
    if ($viewType == 'group') {
        ...
        $byGroupViolations = $groupedViolations->map(function ($group, $key) {
            ...
            return (object)[
                ...
                'violation_time' => $displayTime, // Formatted display time
                'original_time' => $hasMultipleTimes ? $earliestTime : $mostRecentViolation->violation_time, // For sorting/parsing
                ...
                'student_count' => $uniqueStudentCount, // Use unique student count
                'students' => $uniqueStudents, // Unique students only
                'student_names' => $studentNames->implode(', '), // Comma-separated unique names
                ...
                })->sortBy('created_at')->values(), // Detailed violations data sorted by creation time
                'earliest_time' => $earliestTime,
                'latest_time' => $latestTime,
                'has_multiple_times' => $hasMultipleTimes,
                'all_times' => $group->pluck('violation_time')->unique()->values() // All unique times in the group
            ];
        })
        ...
        $paginatedByGroupViolations = new \Illuminate\Pagination\LengthAwarePaginator(
            ...
            [
                'path' => $request->url(),
                'query' => array_merge($request->query(), ['view' => 'group']) // Preserve view type
            ]
        );
        ...
        // Group by offense type and created_at (same day) - each offense type per day gets its own row
        $groupedViolations = ViolationRecord::with(['student', 'offense', 'sanction'])
            ->where('handled_by', 'prefect')
            ->where('status', '!=', 'inactive') // Exclude inactive
            ->orderBy('created_at', 'desc')
        ...
        $byGroupViolations = $groupedViolations->map(function ($group, $key) {
            ...
            // Get date range for this group (use violation dates, not created dates)
            ...
            return (object)[
                ...
                'original_date' => $earliestDate, // For sorting
                'status' => $mostRecentViolation->status,
                'sanction_start_at' => $mostRecentViolation->sanction_start_at,
                'sanction_end_at' => $mostRecentViolation->sanction_end_at,
                'sanction_status' => $mostRecentViolation->sanction_status,
        ...
        $paginatedByGroupViolations = new \Illuminate\Pagination\LengthAwarePaginator(
            ...
            [
                'path' => $request->url(),
                'query' => array_merge($request->query(), ['view' => 'group_per_offense']) // Preserve view type
            ]
        );
        ...
        if ($groupKey) {
            ...
            $violations = ViolationRecord::with(['student', 'offense', 'sanction'])
                ...
                ->whereIn('violation_id', function ($query) use ($groupKey) {
                    $query->select('vr.violation_id')
                        ...
                        ->where('vr.status', '!=', 'inactive') // Exclude inactive
                ...
                ->paginate(10)
                ->withQueryString(); // This preserves the view parameter
            ...
            $paginatedViolations = new \Illuminate\Pagination\LengthAwarePaginator(
                ...
                [
                    'path' => $request->url(),
                    'query' => array_merge($request->query(), ['view' => 'individual']) // Preserve view type
                ]
            );
        ...
        // NEW: Individual view with one row per offense (no merging)
        if ($groupKey) {
            // If coming from a group view, show individual violations for that group
            $violations = ViolationRecord::with(['student', 'offense', 'sanction'])
                ->where('handled_by', 'prefect')
                ->where('status', '!=', 'inactive') // Exclude inactive
                ->where('offense_id', $groupKey) // Group key is offense_id in this case
                ->orderBy('created_at', 'desc')
                ->orderBy('updated_at', 'desc')
                ->orderBy('violation_date', 'desc')
                ->paginate(10)
 ...
 public function indexAnecdotal()
{
    ...
    // Get the actual dates from your violation records filtered by prefect
    $mostRecentViolationDate = DB::table('tbl_violation_record')
        ->where('handled_by', 'prefect')
        ->max('violation_date');
